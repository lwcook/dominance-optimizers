...
class PolySurrogate(object):
    ...
    *Example Declaration*::
    ...
    def __init__(self, dimensions, order=3, poly_type='legendre',
        ...
        self.J_list = [_define_poly_J(p, self.P, a=beta-1, b=alpha-1)
                        for p in self.poly_types]
    ...
    def stats(self):
        '''Compute statistical moments from evaluated coefficients'''

        coeffs = self.coeffs.flatten()
        self.mean = coeffs[0]
        var = sum([c**2 for c in coeffs[1:]])
...
def _define_poly_J(typestr, order, a=2, b=2):
    ...
    if typestr == 'legendre' or typestr == 'uniform':
    ...
    elif typestr == 'jacobi' or typstr == 'beta':
        l,r = -1,1
        a0 = (b-a)/float(a+b+2)
        ab = np.zeros([n,2],float)
        b2a2 = b**2 - a**2
        ...
        if n > 0:
            ab[0,0], ab[0,1] = s*a0+o, 1
        for k in np.arange(2,n+1,1):
            ik = k-1
            ab[ik,0] = s*b2a2/((2*(k-1)+a+b)*(2*k + a+b))+o
            if k == 2:
                numer = float( ((r-l)**2 * (k-1)*(k-1+a)*(k-1+b)) )
                denom = float( ((2*(k-1)+a+b)**2 * (2*(k-1)+a+b+1)) )
            else:
                numer = float( (r-l)**2 * (k-1)*(k-1+a)*(k-1+b)*(k-1+a+b) )
                denom = float( ((2*(k-1)+a+b)**2 *
                               (2*(k-1)+a+b+1)*(2*(k-1)+a+b-1)) )
            ab[ik,1] = numer / denom
